...
以下指标已在专项规格文档中定义，此处仅做索引：

| 指标 | 当前状态 | 参考文档 |
|---|---|---|
| FPGA 型号 | 框架已出，待具体型号冻结 | [[11-FPGA选型与IO预算]] |
| IO 数量与 Bank 分组 | 160~192路可用 / 224~256路预留 | [[11-FPGA选型与IO预算]] |
| DDR 容量与速率 | 框架已出，待冻结 | [[13-带宽与成本预算]] |
| 上行通信方案 | USB3 主链路 + GigE 可选 | [[13-带宽与成本预算]] |
| Pattern 速率与边沿分辨率 | V1 指标已出 | [[10-Pattern引擎时序规格]] |
| Capture 深度与采样时序 | V1 指标已出 | [[12-Capture引擎时序规格]] |
| 扩展连接器 pinout | J1~J6 逐步定义，候选已出 | [[15-板间接口规范]] |
| 电源输入与保护策略 | 本章已覆盖（12V输入/受控辅助电源） | 本节 + [[13-带宽与成本预算]] |
| 板尺寸与堆叠方式 | 待定（依赖连接器选型冻结） | — |
| 散热方案 | 本章已覆盖（散热片+风扇+板温传感器） | 本节 |
